...
Error handling is robust: parsing functions return `ParseError` variants (e.g. `UnknownOption`, `MissingOption`, `InvalidValue`, `TooManyArguments`) which map to exit codes via `exit_codes.mbt`.  Help and version requests are handled by exceptions (`HelpRequested`, `VersionRequested`) that short‑circuit normal execution.
